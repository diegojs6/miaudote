...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  Stream<RegisterState> mapEventToState(
    ...
    yield* event.when(
      load: (username, email, address, fullName, lat, long, contact, birthDate, password) async* {
        yield state.loading();
        ...
        yield await fold.fold(
          (failure) => state.error(_mapRegisterFailureToString(failure)),
          (register) => state.ready(register),
        );
      },
    );
  }

  String _mapRegisterFailureToString(Failure failure) {
